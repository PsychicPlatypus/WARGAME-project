...
class Highscores:
    ...
    def short_scores(self, player, score, victory) -> None:
        ...
        with open("highscores_short.txt", "a") as txt_file:
            txt_file.write(f"{player}: {score} ---- Won: {victory}\n")
    ...
    def long_scores(self, player, score, counter) -> None:
        ...
        with open("highscores_long.txt", "a") as txt_file:
            txt_file.write(f"{player}: {score} ---- Draws: {counter}\n")
    ...
    def sort_highscores(self, high_file, type_sort, backwards):
        ...
        with open(high_file, "w") as write_file:
            write_file.writelines(scores)
    ...
    def shorten_list(self, high_file):
        ...
        if should_del:
            with open(high_file, "w") as write_file:
                write_file.writelines(scores[0:20])
